<div>
    <ul class="uk-breadcrumb">
        ...
        <li class="uk-active"><span><?php echo $app("i18n")->get('Accounts'); ?></span></li>
    </ul>
</div>
...
<div class="uk-margin-top" riot-view>
    ...
    <div class="uk-form uk-clearfix" show="{!loading}">
        ...
        <div class="uk-float-right">
            <a class="uk-button uk-button-primary uk-button-large" href="<?php $app->route('/accounts/create'); ?>">
                <i class="uk-icon-plus-circle uk-icon-justify"></i> <?php echo $app("i18n")->get('Account'); ?>
            </a>
        </div>
    ...
    <div class="uk-text-large uk-text-center uk-margin-large-top uk-text-muted" show="{ !loading && !accounts.length }">
        ...
        <p><?php echo $app("i18n")->get('No users found'); ?></p>
    </div>
            ...
            <tr>
                ...
                <th class="uk-text-small" data-sort="name">
                    <a class="uk-link-muted uk-noselect {sortedBy == 'name' && 'uk-text-primary'}">
                        <?php echo $app("i18n")->get('Name'); ?> <span if="{sortedBy == 'name'}" class="uk-icon-long-arrow-{ sortedOrder == -1 ? 'up':'down'}"></span>
                    </a>
                </th>
                <th class="uk-text-small" width="30%" data-sort="email">
                    <a class="uk-link-muted uk-noselect {sortedBy == 'email' && 'uk-text-primary'}">
                        <?php echo $app("i18n")->get('Email'); ?> <span if="{sortedBy == 'email'}" class="uk-icon-long-arrow-{ sortedOrder == -1 ? 'up':'down'}"></span>
                    </a>
                </th>
                <th class="uk-text-small" width="150" data-sort="group">
                    <a class="uk-link-muted uk-noselect {sortedBy == 'group' && 'uk-text-primary'}">
                        <?php echo $app("i18n")->get('Group'); ?> <span if="{sortedBy == 'group'}" class="uk-icon-long-arrow-{ sortedOrder == -1 ? 'up':'down'}"></span>
                    </a>
                </th>
        ...
        <tbody>
            <tr each="{account, $index in accounts}" if="{ infilter(account) }">
                <td class="uk-text-center">
                    <a class="uk-link-muted" href="<?php $app->route('/accounts/account'); ?>/{ account._id }" title="<?php echo $app("i18n")->get('Edit account'); ?>">
                    ...
                    <span data-uk-dropdown="pos:'bottom-right'">
                        ...
                        <div class="uk-dropdown">
                            <ul class="uk-nav uk-nav-dropdown uk-dropdown-close">
                                ...
                                <li><a href="<?php $app->route('/accounts/account'); ?>/{ account._id }"><?php echo $app("i18n")->get('Edit'); ?></a></li>
    ...
    <script type="view/script">
        ...
        this.accounts = [];
        this.groups   = <?php echo  json_encode($groups) ; ?>;
